...
public class Rook extends Piece{
	...
	public Collection<Move> validateMove(GameModel gameModel, int x, int y) {
		...
		//List<Move> valideMoves = new ArrayList<>();
		...
		for(int line = 0; line < CANDIDATE_MOVE_COORDINATES_LINE.length; line++)
			for(int column = 0; column < CANDIDATE_MOVE_COORDINATES_COLUMN.length; column++) {
				for(int i=1;i<8;i++) {
					if(Math.abs(CANDIDATE_MOVE_COORDINATES_LINE[line]) != Math.abs(CANDIDATE_MOVE_COORDINATES_COLUMN[column]) &&
						x+i*CANDIDATE_MOVE_COORDINATES_LINE[line]>=0 && x+i*CANDIDATE_MOVE_COORDINATES_LINE[line]<7 && 
